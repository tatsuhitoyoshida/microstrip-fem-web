...
The `research/src/fem-fullwave/` tree implements a complete vector full-wave
...
| Path                                           | Role                                                                                                |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `research/src/fem-fullwave/edge-dofs.ts`                | Edge-DoF enumeration + orientation for Nédélec elements.                                            |
| `research/src/fem-fullwave/nedelec.ts`                  | Whitney 1-form element matrices (curl-curl, vector mass).                                           |
| `research/src/fem-fullwave/vector-assembly.ts`          | Real-isotropic edge-DoF block assembly (homogeneous / dielectric-loaded paths).                     |
| `research/src/fem-fullwave/assembly.ts`                 | Real-weight P1 nodal stiffness + mass (scalar block).                                               |
| `research/src/fem-fullwave/boundary.ts`                 | PEC-restriction utilities (real and complex variants).                                              |
| `research/src/fem-fullwave/gradient.ts`                 | Discrete gradient operator G + M-orthogonal deflator.                                               |
| `research/src/fem-fullwave/eigsolve.ts`                 | Inverse-iteration + shift-invert MINRES eigensolver for real symmetric GEPs.                        |
| `research/src/fem-fullwave/minres.ts`                   | Real symmetric indefinite linear solver (Paige-Saunders).                                           |
| `research/src/fem-fullwave/mixed-assembly.ts`           | Real (E_t, E_z) block assembly with Schur convention.                                               |
| `research/src/fem-fullwave/schur.ts`                    | M̃ = M_t − C_tz K_n⁻¹ C_tz^T (real) via inner CG.                                                    |
| `research/src/fem-fullwave/complex-sparse.ts`           | Complex CSR + interleaved storage + `cdot` / `cdotH` inner products.                                |
| `research/src/fem-fullwave/complex-solver.ts`           | Complex Bi-CGSTAB with Jacobi preconditioning.                                                      |
| `research/src/fem-fullwave/complex-vector-assembly.ts`  | Anisotropic / complex edge-DoF blocks (curl-curl, mass, coupling).                                  |
| `research/src/fem-fullwave/complex-scalar-assembly.ts`  | Anisotropic / complex P1 nodal blocks.                                                              |
| `research/src/fem-fullwave/complex-eigsolve.ts`         | Shift-invert eigensolver on complex symmetric GEPs.                                                 |
| `research/src/fem-fullwave/complex-schur.ts`            | Complex M̃ via inner Bi-CGSTAB.                                                                      |
| `research/src/fem-fullwave/complex-gradient.ts`         | Complex M-orthogonal gradient deflator.                                                             |
| `research/src/fem-fullwave/pml.ts`                      | SC-PML 1-D profiles + 2-D Cartesian config + tensor-weight factories.                               |
| `research/src/fem-fullwave/mixed-pml-assembly.ts`       | PML-aware (E_t, E_z) block assembly.                                                                |
| `research/src/fem-fullwave/pml-eigensolve.ts`           | End-to-end PML pipeline wrapper (assemble → restrict → Schur → deflate → eigsolve → recover E_z).   |
| `research/src/fem-fullwave/microstrip-pml.ts`           | Microstrip geometry → triangle-wasm mesh → PML pipeline → β².                                       |
| `research/src/fem-fullwave/microstrip-z0.ts`            | ε_eff(f) + Z₀ (V-P definition) from the converged eigenpair.                                        |
...
| Test file                                       | Coverage                                                                       |
| ----------------------------------------------- | ------------------------------------------------------------------------------ |
| `research/tests/fem-fullwave/nedelec.test.ts`            | Whitney 1-form element matrices, curl identity, mass / curl-curl symmetry.    |
| `research/tests/fem-fullwave/vector-assembly.test.ts`    | Global edge-DoF assembly: symmetry, ∇×constant = 0, coupling block shape.     |
| `research/tests/fem-fullwave/eigsolve.test.ts`           | Inverse-iteration eigensolver on toy real-symmetric GEPs.                     |
| `research/tests/fem-fullwave/minres.test.ts`             | Real MINRES on symmetric indefinite systems.                                  |
| `research/tests/fem-fullwave/gradient.test.ts`           | Discrete curl-grad identity, deflator idempotency + M-orthogonality.          |
| `research/tests/fem-fullwave/shift-invert.test.ts`       | Real shift-invert eigensolver including with the deflator.                    |
| `research/tests/fem-fullwave/closed-waveguide.test.ts`   | TE_10 in closed PEC 2×1 box matches `(π/a)²`; mesh refinement.                |
| `research/tests/fem-fullwave/mixed-waveguide.test.ts`    | Mixed system Schur: homogeneous + dielectric-loaded closed box.               |
| `research/tests/fem-fullwave/complex-sparse.test.ts`     | Complex CSR, both inner products, transpose vs adjoint, BLAS-1 ops.           |
| `research/tests/fem-fullwave/complex-solver.test.ts`     | Complex Bi-CGSTAB: real SPD, complex symmetric, non-symmetric, indefinite.    |
| `research/tests/fem-fullwave/complex-vector-assembly.test.ts` | Anisotropic / complex edge-DoF blocks reduce to real isotropic at κ=0.   |
| `research/tests/fem-fullwave/complex-scalar-assembly.test.ts` | Same for P1 nodal blocks.                                                 |
| `research/tests/fem-fullwave/pml.test.ts`                | SC-PML profiles + weight factories + `noPml()` reduction.                     |
| `research/tests/fem-fullwave/mixed-pml-assembly.test.ts` | PML-aware 4-block assembly reduces to real path at κ=0.                       |
| `research/tests/fem-fullwave/complex-eigsolve.test.ts`   | Complex shift-invert eigensolver on synthetic complex symmetric GEPs.         |
| `research/tests/fem-fullwave/closed-waveguide-pml.test.ts` | End-to-end κ=0 pipeline through PML reproduces real-path β².                |
| `research/tests/fem-fullwave/pml-eigensolve.test.ts`     | `solveMixedSystemPml` wrapper regression on closed-domain answers.            |
| `research/tests/fem-fullwave/microstrip-pml.test.ts`     | FR-4 microstrip @ 20 GHz: ε_eff vs KJ-dispersive, V-P Z₀ smoke test.          |
| `research/tests/fem-fullwave/microstrip-dispersion.test.ts` | FR-4 multi-frequency (20 / 30 GHz): ε_eff(f) tracks KJ within 0.3 %.       |
