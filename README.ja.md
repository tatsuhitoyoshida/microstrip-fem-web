...
> 本番計算機(v0.1 経路、Hammerstad–Jensen と 2 % 以内一致)。
> ベクトル全波 (Nédélec エッジ要素 + SC-PML 切り捨て) 固有値ソルバも
> 同時開発したが、UI 表示速度を保つ粗メッシュでの Z₀ 絶対値が参考値
> から ~30 % ずれていたため、v0.2 では **本番 UI から外して棚上げ**。
> 数学パイプライン自体は end-to-end 検証済み(ε_eff(FEM) が KJ と
> 0.3 % 以内一致)。コード・テスト・実験 UI ページは
> [`research/`](research/) に保存され、開発再開はクリーンに行える。
> 棚上げの根本原因(より強力な前処理が必要)は
> [`docs/roadmap.md`](docs/roadmap.md) に記録。
...
### 棚上げ: 実験版 Full-wave PML ソルバ

ベクトル Helmholtz (Nédélec エッジ要素 + SC-PML 切り捨て) 固有値
ソルバを完全実装したが、UI 表示速度を保つ粗メッシュでの Z₀ 絶対値が
信頼可能でなかったため、v0.2 UI から外している。数学コードは
[`research/`](research/) に専用テストスイート
(`npm run test:research`) と共に保存。再開手順は
[`research/README.md`](research/README.md)。定式化は
[`docs/theory.ja.md` §13](docs/theory.ja.md)、本番昇格に必要な
作業は [`docs/roadmap.md`](docs/roadmap.md) を参照。
